...
def load_yolo_annotation(file_path, img_width, img_height):
    """ฟังก์ชันโหลดข้อมูลจากไฟล์ YOLO annotation"""
    if os.path.exists(file_path):
        ...
        for annotation in annotations:
            ...
            rects.append({
                ...
                "width": x_max - x_min,
                "height": y_max - y_min,
                "label": st.session_state["labels"][class_index]
            })
        return rects
    return []
...
def run():
    ...
    # โหลด annotation ถ้ามี
    annotation_file_path = os.path.join(save_dir, img_file_name.split(".")[0] + ".txt")
    if save_format == "YOLO":
        rects = load_yolo_annotation(annotation_file_path, img.width, img.height)
    else:
        rects = resized_rects

    is_annotated = os.path.exists(annotation_file_path)
    # แสดงภาพและสัญลักษณ์ติ้กถูกหากมีการอนุมัติแล้ว
    # st.image(resized_img, caption=f"{img_file_name} - {'✓' if is_annotated else 'Not Annotated'}", use_column_width=True)
    if is_annotated:
        st.markdown('<span style="color:blue;">✔</span>', unsafe_allow_html=True)

    rects = st_img_label(resized_img, box_color="red", rects=rects)
